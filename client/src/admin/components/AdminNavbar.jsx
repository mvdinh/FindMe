...
const AdminNavbarPanel = ({ onNavigate }) => {
  ...
  const navigationItems = [
    ...
    { name: 'Quản lý HR', href: '/admin/hr-management' },
    { name: 'Tài khoản', href: '/admin/accounts' },
    { name: 'Tất cả việc làm', href: '/admin/jobs' },
    { name: 'Duyệt yêu cầu HR', href: '/admin/job-status-requests' }
  ];
  ...
  const profileActive = p === '/admin' || p === '/admin/dashboard';

  const isItemActive = item => {
    if (item.href === '/admin/dashboard') return p === '/admin' || p === '/admin/dashboard';
    return p === item.href || (item.href !== '/admin/dashboard' && p.startsWith(item.href));
  };
    ...
    <div className="flex h-full min-h-0 flex-col border-border bg-sidebar text-sidebar-foreground">
      ...
      <div className="shrink-0 space-y-3 border-t border-border bg-sidebar/50 p-3">
        ...
        <Button variant="outline" className="relative h-auto min-h-11 w-full touch-manipulation gap-2 py-2.5 font-['Roboto']" asChild>
          <Link to="/admin/notifications" onClick={closeMobile}>
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate font-['Open_Sans'] text-sm font-semibold text-foreground">
              {user?.firstName && user?.lastName ? `${user.firstName} ${user.lastName}` : 'Quản trị viên'}
            </p>
