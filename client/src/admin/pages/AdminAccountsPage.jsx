...
import { AlertCircle, ChevronLeft, ChevronRight, Eye, Lock, LockOpen, Loader2 } from 'lucide-react';
...
const AdminAccountsPage = () => {
  ...
  const buildQuery = useCallback(() => {
    ...
    if (activeFilter === 'true' || activeFilter === 'false') params.set('isActive', activeFilter);
  ...
  const toggleLoginActive = async row => {
    ...
    try {
      ...
      setError(e.response?.data?.message || e.message || 'Không cập nhật được.');
    } finally {
      setBusyUserId(null);
    }
  };
      ...
      <div className={ADMIN_PAGE}>
        <div className={ADMIN_PAGE_HEADER}>
          <div className="min-w-0 flex-1">
            ...
            <p className={ADMIN_SUBTITLE}>
              Xem danh sách HR và ứng viên trong phạm vi được phép; bật/tắt đăng nhập khi cần (không quản lý tài khoản quản trị).
            </p>
          </div>
        </div>
        ...
        <Card className="mb-6 shadow-sm">
          <CardContent className="flex flex-col flex-wrap gap-3 p-4 lg:flex-row lg:items-end">
            ...
            <Button type="button" className="font-['Roboto']" onClick={() => loadUsers()}>
              Làm mới
            </Button>
          </CardContent>
        </Card>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    {['STT', 'Họ tên', 'Email', 'Vai trò', 'Trạng thái', 'Thao tác'].map((h, i) => (
                      <TableHead
                        key={h}
                        className={cn(
                ...
                <TableBody>
                  {users.length === 0 ? (
                    <TableRow className="hover:bg-transparent">
                      <TableCell colSpan={6} className="px-4 py-12 text-center text-sm text-muted-foreground font-['Roboto']">
                          ...
                          <div className="flex flex-col gap-1">
                            <Badge
                              ...
                              className={cn(
                                ...
                                row.accountStatus === 'active'
                                  ...
                                  : row.accountStatus === 'inactive' || row.accountStatus === 'suspended'
                            ...
                            <span
                              className={cn(
                                ...
                                row.isActive !== false ? 'text-emerald-600 dark:text-emerald-400' : 'text-destructive'
                              )}
                            >
                              {row.isActive !== false ? 'Đang bật' : 'Đã tắt'}
                            </span>
                          </div>
                        </TableCell>
                        <TableCell className="px-4 py-3">
                          <div className="flex flex-wrap items-center gap-1.5">
                            ...
                            <Button
                              ...
                              title={row.isActive !== false ? 'Vô hiệu hóa đăng nhập' : 'Kích hoạt đăng nhập'}
                              aria-label={row.isActive !== false ? 'Vô hiệu hóa đăng nhập' : 'Kích hoạt đăng nhập'}
                              onClick={() => toggleLoginActive(row)}
                              disabled={
                                !!busyUserId ||
                                (authUser?.id && String(authUser.id) === String(row.id) && row.isActive !== false)
                              }
                            >
      ...
      <AdminModal
        ...
        footer={
          <div className="flex justify-end">
            <Button type="button" className="px-5 py-2.5 text-sm" onClick={closeView}>
              Đóng
            </Button>
          </div>
        }
        ...
        {viewing?.skeleton ? (
          ...
          <div className="space-y-3 font-['Roboto'] text-sm">
            <dl className="grid grid-cols-2 gap-x-3 gap-y-2">
              ...
              <dd className="text-foreground">{ROLE_LABEL[viewing.role] || viewing.role}</dd>
              ...
              <dd className="text-foreground">{STATUS_LABEL[viewing.accountStatus] || viewing.accountStatus}</dd>
              <dt className="text-muted-foreground">Đăng nhập</dt>
              <dd className="text-foreground">{viewing.isActive !== false ? 'Đang bật' : 'Đã tắt'}</dd>
              <dt className="text-muted-foreground">Tạo lúc</dt>
              <dd className="text-foreground">
                {viewing.createdAt ? formatDateTimeVN(viewing.createdAt) : '—'}
              </dd>
              <dt className="text-muted-foreground">Đăng nhập gần nhất</dt>
              <dd className="text-foreground">
                {viewing.lastLogin ? formatDateTimeVN(viewing.lastLogin) : '—'}
              </dd>
            </dl>
          </div>
        ) : null}
