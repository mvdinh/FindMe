...
import { AlertCircle, ChevronLeft, ChevronRight, Copy, Eye, EyeOff, KeyRound, Pencil, UserCheck, UserX } from 'lucide-react';
...
const RecruiterManagementPage = () => {
  ...
  const loadRecruiterUsers = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const response = await makeJsonRequest(`/api/admin/recruiter?${qs}`);
      if (Array.isArray(response)) {
        ...
        if (response.pagination) {
          ...
          setRecruiterPagination({
            currentPage: p.currentPage ?? recruiterPage,
      ...
      setError('Không thể tải danh sách Recruiter. Vui lòng thử lại.');
    } finally {
      setLoading(false);
    }
  }, [makeJsonRequest, recruiterPage]);
  ...
  const handleCopyPassword = () => {
    if (newRecruiter.password) {
      navigator.clipboard.writeText(newRecruiter.password);
      toast.success('Đã sao chép mật khẩu vào bộ nhớ tạm!');
    }
  };
  const handleAddRecruiter = async () => {
    const department = newRecruiter.department === 'custom' ? newRecruiter.customDepartment : newRecruiter.department;
    if (newRecruiter.firstName && newRecruiter.lastName && newRecruiter.email && department && newRecruiter.password) {
      try {
        setSubmitting(true);
        setError(null);
        const response = await makeJsonRequest('/api/admin/recruiter', {
          method: 'POST',
          headers: {
        ...
        if (response && response.recruiter) {
          ...
          toast.success('Đã tạo tài khoản Nhà tuyển dụng thành công!');
        }
      } catch (error) {
        console.error('Error creating Recruiter user:', error);
        toast.error(error.message || 'Không thể tạo tài khoản Nhà tuyển dụng. Vui lòng thử lại.');
      } finally {
        setSubmitting(false);
      }
    } else {
  ...
  const handleEditRecruiter = recruiter => {
    ...
    const isCustomDepartment = !departmentOptions.includes(recruiter.department);
    ...
    setNewRecruiter({
      ...
      department: isCustomDepartment ? 'custom' : recruiter.department,
      customDepartment: isCustomDepartment ? recruiter.department : '',
      password: ''
    });
  ...
  const handleUpdateRecruiter = async () => {
    const department = newRecruiter.department === 'custom' ? newRecruiter.customDepartment : newRecruiter.department;
    if (newRecruiter.firstName && newRecruiter.lastName && department && editingRecruiter) {
      try {
        setSubmitting(true);
        setError(null);
        const updateData = {
        ...
        const response = await makeJsonRequest(`/api/admin/recruiter/${editingRecruiter.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        if (response && response.recruiter) {
          setRecruiters(recruiters.map(recruiter => recruiter.id === editingRecruiter.id ? response.recruiter : recruiter));
          ...
          setShowAddModal(false);
          setShowPassword(false);
          toast.success('Đã cập nhật thông tin Recruiter thành công!');
        }
      } catch (error) {
        console.error('Error updating Recruiter user:', error);
        toast.error(error.message || 'Không thể cập nhật tài khoản Nhà tuyển dụng. Vui lòng thử lại.');
      } finally {
        setSubmitting(false);
      }
    } else {
  ...
  const handleToggleStatus = async recruiterId => {
    try {
      setError(null);
      const recruiter = recruiters.find(h => h.id === recruiterId);
      const newStatus = recruiter.status === 'active' ? 'inactive' : 'active';
      const response = await makeJsonRequest(`/api/admin/recruiter/${recruiterId}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (response && response.recruiter) {
        const safeHr = {
          ...response.recruiter,
          jobsPosted: Number(response.recruiter.jobsPosted) || 0,
          interviewScheduled: Number(response.recruiter.interviewScheduled) || 0,
          interviewPassed: Number(response.recruiter.interviewPassed) || 0
        };
        setRecruiters(recruiters.map(recruiter => recruiter.id === recruiterId ? safeHr : recruiter));
        toast.success(newStatus === 'active' ? 'Đã kích hoạt tài khoản Nhà tuyển dụng!' : 'Đã vô hiệu hóa tài khoản Nhà tuyển dụng!');
      }
    } catch (error) {
      console.error('Error toggling Recruiter status:', error);
      toast.error(error.message || 'Không thể cập nhật trạng thái Recruiter. Vui lòng thử lại.');
    }
  };
  const closeModal = () => {
    setShowAddModal(false);
  ...
  const activeRecruiters = recruiterSummary.activeHR;
  const totalJobsPosted = recruiterSummary.totalJobsPosted;
  const totalInterviewScheduled = recruiterSummary.interviewScheduled;
  const totalInterviewPassed = recruiterSummary.interviewPassed;
  const recruiterPageLimit = recruiterPagination.limit || 20;
  return <AdminLayout>
      <div className={ADMIN_PAGE}>
        <div className={ADMIN_PAGE_HEADER}>
          <div className="min-w-0 flex-1">
            <h1 className={ADMIN_H1}>Quản lý Nhà tuyển dụng</h1>
            <p className={ADMIN_SUBTITLE}>Quản lý thành viên Nhà tuyển dụng trong tổ chức của bạn.</p>
          </div>
          ...
          <>
            <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-4">
              <Card>
                <CardContent className="flex items-center gap-4 pt-6">
                  ...
                  <div>
                    <p className="font-['Roboto'] text-sm font-medium text-muted-foreground">Nhà tuyển dụng đang hoạt động</p>
                    <p className="font-['Open_Sans'] text-2xl font-bold text-foreground">{activeRecruiters}</p>
                  </div>
                </CardContent>
              </Card>
              <Card>
            ...
            <Card className="overflow-hidden">
              <CardHeader className="border-b">
                <CardTitle className="font-['Open_Sans'] text-lg">Nhà tuyển dụng ({recruiters.length})</CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="font-['Roboto']">Thông tin Recruiter</TableHead>
                      ...
                      {recruiters.map((recruiter, index) => (
                        <TableRow key={recruiter.id}>
                          <TableCell className="text-center font-['Roboto'] text-sm tabular-nums text-muted-foreground">
                            {(recruiterPage - 1) * recruiterPageLimit + index + 1}
                          </TableCell>
                          <TableCell>
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <div className="font-['Open_Sans'] text-sm font-medium text-foreground">{recruiter.name}</div>
                                <div className="font-['Roboto'] text-sm text-muted-foreground">{recruiter.email}</div>
                              </div>
                            </div>
                          </TableCell>
                          <TableCell>
                            ...
                            >
                              {recruiter.status === 'active' ? 'đang hoạt động' : 'không hoạt động'}
                            </Badge>
                          </TableCell>
                          <TableCell className="font-['Roboto'] text-sm text-foreground">
                            <div className="space-y-0.5">
                              <div>{recruiter.jobsPosted} việc làm đã đăng</div>
                              <div className="text-muted-foreground">{recruiter.interviewScheduled ?? 0} đã lên lịch</div>
                              <div className="text-muted-foreground">{recruiter.interviewPassed ?? 0} pass phỏng vấn</div>
                            </div>
                          </TableCell>
                          <TableCell className="font-['Roboto'] text-sm text-muted-foreground">{formatDateVN(recruiter.dateJoined) || '—'}</TableCell>
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-1">
                              <Button type="button" variant="outline" size="icon-sm" title="Chỉnh sửa Nhà tuyển dụng" aria-label="Chỉnh sửa Nhà tuyển dụng" onClick={() => handleEditRecruiter(recruiter)}>
                              ...
                              <Button
                                ...
                                variant="outline"
                                size="icon-sm"
                                title={recruiter.status === 'active' ? 'Vô hiệu hóa Nhà tuyển dụng' : 'Kích hoạt Nhà tuyển dụng'}
                                aria-label={recruiter.status === 'active' ? 'Vô hiệu hóa Nhà tuyển dụng' : 'Kích hoạt Nhà tuyển dụng'}
                                className={recruiter.status === 'active' ? 'text-emerald-600' : ''}
                                onClick={() => handleToggleStatus(recruiter.id)}
                              >
                                {recruiter.status === 'active' ? <UserX className="size-4" /> : <UserCheck className="size-4" />}
                              </Button>
                ...
                <Pagination
                  ...
                  totalPages={recruiterPagination.totalPages}
                  onPageChange={(p) => setRecruiterPage(p)}
                  loading={loading}
                  totalItems={recruiterPagination.totalItems}
                  limit={recruiterPageLimit}
                  itemLabel="Recruiter"
                />
              </CardContent>
            </Card>
        ...
        <AdminModal
          open={showAddModal}
          onClose={closeModal}
          title={editingRecruiter ? 'Chỉnh sửa Nhà tuyển dụng' : 'Thêm Nhà tuyển dụng mới'}
          size="lg"
          footer={
            <div className="flex justify-end gap-2">
              ...
              <Button
                type="button"
                onClick={editingRecruiter ? handleUpdateRecruiter : handleAddRecruiter}
                disabled={
                  submitting ||
                  !newRecruiter.firstName ||
                  !newRecruiter.lastName ||
                  !newRecruiter.email ||
                  (newRecruiter.department === 'custom' ? !newRecruiter.customDepartment : !newRecruiter.department) ||
                  (!editingRecruiter && !newRecruiter.password)
                }
              >
                {submitting ? (editingRecruiter ? 'Đang cập nhật...' : 'Đang thêm...') : editingRecruiter ? 'Cập nhật Recruiter' : 'Thêm Nhà tuyển dụng'}
              </Button>
            </div>
          }
        >
          ...
          <fieldset disabled={submitting} className="space-y-4 min-w-0 border-0 p-0 m-0 disabled:opacity-[0.9]">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="recruiter-first" className="font-['Roboto']">
                  Tên
                </Label>
                <Input
                  id="recruiter-first"
              ...
              <div className="space-y-2">
                <Label htmlFor="recruiter-last" className="font-['Roboto']">
                  Họ
                </Label>
                <Input
                  id="recruiter-last"
            ...
            <div className="space-y-2">
              <Label htmlFor="recruiter-email" className="font-['Roboto']">
                Địa chỉ email
              </Label>
              <Input
                id="recruiter-email"
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-3">
                <Select
                  value={newRecruiter.department || '__none'}
                  onValueChange={v =>
                    setNewRecruiter({
                      ...newRecruiter,
                      department: v === '__none' ? '' : v,
                      customDepartment: v === 'custom' ? newRecruiter.customDepartment : ''
                    })
                  }
                >
                ...
                {newRecruiter.department === 'custom' && (
                  <Input
                    value={newRecruiter.customDepartment}
                    onChange={e => setNewRecruiter({ ...newRecruiter, customDepartment: e.target.value })}
                    className="h-10 rounded-full font-['Roboto']"
            ...
            {!editingRecruiter && (
              <div className="space-y-2">
                <Label htmlFor="recruiter-password" className="font-['Roboto']">
                  Mật khẩu
                </Label>
                ...
                {newRecruiter.password && (
                  <p className="font-['Roboto'] text-xs text-muted-foreground">
                    Độ mạnh mật khẩu:{' '}
                    <span className="font-medium text-foreground">{newRecruiter.password.length >= 8 ? 'Mạnh' : 'Yếu'}</span>
                  </p>
                )}
              </div>
            )}
