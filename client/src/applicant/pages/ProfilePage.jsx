            ...
            <div className="rounded-xl border border-border bg-card p-8 shadow-sm transition-colors duration-300">
              ...
              <div className="mb-8 pb-8 border-b border-border transition-colors duration-300">
                <div className="mb-6 flex items-center justify-between">
                  <h2 className="text-xl font-semibold text-foreground font-['Open_Sans'] transition-colors duration-300">
                    Học vấn
                  </h2>
                  {isEditing && <Button type="button" size="sm" className="gap-2 font-['Roboto']" onClick={() => openAddModal('education')}>
                      <Plus className="size-4" />
                      Thêm học vấn
                    </Button>}
                </div>
                
                {formData.education && formData.education.length === 0 ? <div className="text-center py-8 text-muted-foreground font-['Roboto'] transition-colors duration-300">
                    Chưa thêm thông tin học vấn
                  </div> : (formData.education || []).map(edu => <div key={edu.id} className="flex items-start space-x-4 mb-4 group">
                      <div className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-muted transition-colors duration-300">
                        <GraduationCap className="size-4 text-muted-foreground" />
                      </div>
                      <div className="flex-1">
                        <h3 className="font-medium text-foreground font-['Open_Sans'] transition-colors duration-300">{edu.institution}</h3>
                        <p className="text-sm text-muted-foreground font-['Roboto'] mb-1 transition-colors duration-300">{normalizeProfileText(edu.degree)}</p>
                        <p className="text-sm text-muted-foreground font-['Roboto'] mb-1 transition-colors duration-300">Tốt nghiệp: {edu.graduationDate}</p>
                        {normalizeProfileText(edu.description) && <p className="text-sm text-foreground font-['Roboto'] transition-colors duration-300">{normalizeProfileText(edu.description)}</p>}
                      </div>
                      {isEditing && <div className="flex items-center space-x-1 opacity-0 transition-opacity group-hover:opacity-100">
                          <Button type="button" variant="ghost" size="icon" className="shrink-0 text-muted-foreground hover:text-foreground" onClick={() => openEditModal('education', edu)} title="Sửa học vấn">
                            <Pencil className="size-4" />
                          </Button>
                          <Button type="button" variant="ghost" size="icon" className="shrink-0 text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => handleArrayRemove('education', edu.id)} title="Xóa học vấn">
              ...
              <div className="mb-8 pb-8 border-b border-border transition-colors duration-300">
                <div className="mb-6 flex items-center justify-between">
                  ...
                  {isEditing && <Button type="button" size="sm" className="gap-2 font-['Roboto']" onClick={() => openAddModal('workExperience')}>
                      <Plus className="size-4" />
                      Thêm kinh nghiệm
                    </Button>}
                </div>
                
                {formData.workExperience && formData.workExperience.length === 0 ? <div className="text-center py-8 text-muted-foreground font-['Roboto'] transition-colors duration-300">
                    Chưa thêm kinh nghiệm làm việc
                  </div> : (formData.workExperience || []).map(work => <div key={work.id} className="flex items-start space-x-4 mb-4 group">
                      <div className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-muted transition-colors duration-300">
                        <Briefcase className="size-4 text-muted-foreground" />
                      </div>
                      <div className="flex-1">
                        <h3 className="font-medium text-foreground font-['Open_Sans'] transition-colors duration-300">{work.company}</h3>
                        <p className="text-sm text-muted-foreground font-['Roboto'] mb-1 transition-colors duration-300">{work.position}</p>
                        <p className="text-sm text-muted-foreground font-['Roboto'] mb-1 transition-colors duration-300">{work.duration}</p>
                        {normalizeProfileText(work.description) && <p className="text-sm text-foreground font-['Roboto'] transition-colors duration-300">{normalizeProfileText(work.description)}</p>}
                      </div>
                      {isEditing && <div className="flex items-center space-x-1 opacity-0 transition-opacity group-hover:opacity-100">
                          <Button type="button" variant="ghost" size="icon" className="shrink-0 text-muted-foreground hover:text-foreground" onClick={() => openEditModal('workExperience', work)} title="Sửa kinh nghiệm">
                            <Pencil className="size-4" />
                          </Button>
                          <Button type="button" variant="ghost" size="icon" className="shrink-0 text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => handleArrayRemove('workExperience', work.id)} title="Xóa kinh nghiệm">
                            <Trash2 className="size-4" />
                          </Button>
                        </div>}
                    </div>)}
              ...
              <div className="mb-8 pb-8 border-b border-border transition-colors duration-300">
                ...
                {isEditing ? <div className="space-y-4">
                    {formData.skills && formData.skills.length > 0 && <div className="flex min-h-[60px] flex-wrap gap-2 rounded-lg border border-border bg-muted/40 p-3 transition-colors duration-300">
                        {formData.skills.map((skill, index) => <span key={index} className="inline-flex items-center gap-0.5">
                            <Badge variant="default" className="font-['Roboto'] font-normal">
                              {skill}
                            </Badge>
                            <Button type="button" variant="ghost" size="icon-xs" className="text-muted-foreground hover:text-foreground" onClick={() => removeSkill(skill)} aria-label={`Xóa ${skill}`}>
                              <X className="size-3" />
                            </Button>
                          </span>)}
                      </div>}
                    ...
                    <div className="flex gap-2">
                      <input type="text" placeholder="Nhập kỹ năng và nhấn Enter" className={HR_INPUT} onKeyPress={e => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
                  ...
                  if (skill) {
                    ...
                    <div>
                      <p className="mb-2 font-['Roboto'] text-sm text-muted-foreground transition-colors duration-300">Kỹ năng phổ biến (bấm để thêm):</p>
                      <div className="flex max-h-32 flex-wrap gap-2 overflow-y-auto">
                        {suggestedSkills.filter(skill => !formData.skills.includes(skill)).slice(0, 30).map(skill => <Button key={skill} type="button" variant="outline" size="sm" className="h-auto rounded-full px-3 py-1 font-['Roboto'] font-normal text-muted-foreground" onClick={() => addSkill(skill)}>
                            + {skill}
                          </Button>)}
                      </div>
                    </div>
                    ...
                    <p className="text-sm text-muted-foreground font-['Roboto'] transition-colors duration-300">
                      Thêm các kỹ năng thể hiện tốt nhất chuyên môn của bạn. Bạn có thể nhập kỹ năng tùy ý hoặc chọn từ danh sách gợi ý.
                    </p>
                  </div> : <div>
                    {formData.skills && formData.skills.length > 0 ? <div className="flex flex-wrap gap-2">
                        {formData.skills.map((skill, index) => <Badge key={index} variant="secondary" className="font-['Roboto'] font-normal">
                            {skill}
                          </Badge>)}
                      </div> : <div className="text-center py-8 text-muted-foreground font-['Roboto'] transition-colors duration-300">
              ...
              <div>
                <div className="flex items-center justify-between mb-6">
                  ...
                  {isEditing && <Button type="button" size="sm" className="gap-2 font-['Roboto']" onClick={() => openAddModal('projects')}>
                      <Plus className="size-4" />
                      Thêm dự án
                    </Button>}
                </div>
                
                {formData.projects && formData.projects.length === 0 ? <div className="text-center py-8 text-muted-foreground font-['Roboto'] transition-colors duration-300">
                    Chưa thêm dự án nào
                  </div> : (formData.projects || []).map(project => <div key={project.id} className="flex items-start space-x-4 mb-4 group">
                      <div className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-muted transition-colors duration-300">
                        <FolderCode className="size-4 text-muted-foreground" />
                      </div>
                      <div className="flex-1">
                        <h3 className="font-medium text-foreground font-['Open_Sans'] transition-colors duration-300">{project.name}</h3>
                        <p className="text-sm text-muted-foreground font-['Roboto'] mb-1 transition-colors duration-300">{project.technologies}</p>
                        <p className="text-sm text-foreground font-['Roboto'] transition-colors duration-300">{project.description}</p>
                      </div>
                      {isEditing && <div className="flex items-center space-x-1 opacity-0 transition-opacity group-hover:opacity-100">
                          <Button type="button" variant="ghost" size="icon" className="shrink-0 text-muted-foreground hover:text-foreground" onClick={() => openEditModal('projects', project)} title="Sửa dự án">
                            <Pencil className="size-4" />
                          </Button>
                          <Button type="button" variant="ghost" size="icon" className="shrink-0 text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => handleArrayRemove('projects', project.id)} title="Xóa dự án">
                            <Trash2 className="size-4" />
                          </Button>
                        </div>}
                    </div>)}
