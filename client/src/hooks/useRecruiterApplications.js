...
export const useRecruiterApplications = () => {
  ...
  const fetchApplications = useCallback(
    async (params = {}, options = {}) => {
      ...
      try {
        ...
        const response = await makeJsonRequest(`/api/recruiter/applications?${queryParams}`);
        if (response.success) {
          const responseData = {
  ...
  const updateApplicationStatus = useCallback(
    async (applicationId, newStatus, notes = '', scheduleMeta = {}) => {
      try {
        const response = await makeJsonRequest(`/api/recruiter/applications/${applicationId}/status`, {
          method: 'PUT',
          body: JSON.stringify({
            status: newStatus,
            notes,
