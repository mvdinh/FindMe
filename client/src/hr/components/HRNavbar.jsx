...
const HRSidebarPanel = ({ onNavigate }) => {
  ...
  const navigationItems = [
    { name: 'Tổng quan', href: '/hr/dashboard' },
    { name: 'Tin tuyển dụng', href: '/hr/jobs' },
    { name: 'Hồ sơ ứng viên', href: '/hr/applications' }
  ];
  ...
  const profileActive = p === '/hr/profile' || p.startsWith('/hr/profile');

  const isItemActive = item => {
    if (item.href === '/hr/dashboard') return p === '/hr/dashboard';
    return p === item.href || (item.href !== '/hr/dashboard' && p.startsWith(item.href));
  };
    ...
    <div className="flex h-full min-h-0 flex-col border-border bg-sidebar text-sidebar-foreground">
      ...
      <div className="shrink-0 space-y-3 border-t border-border bg-sidebar/50 p-3">
        ...
        <Button variant="outline" className="relative h-auto min-h-11 w-full touch-manipulation gap-2 py-2.5 font-['Roboto']" asChild>
          <Link to="/hr/notifications" onClick={closeMobile}>
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate font-['Open_Sans'] text-sm font-semibold text-foreground">
              {user?.firstName && user?.lastName ? `${user.firstName} ${user.lastName}` : 'Nhân sự tuyển dụng'}
            </p>
