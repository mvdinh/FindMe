...
import { HR_PAGE, HR_PAGE_HEADER, HR_H1, HR_SUBTITLE, HR_FILTER_CHIPS, HR_NATIVE_FIELD } from '../hrLayoutClasses';
import { HR_INPUT, HR_TEXTAREA } from '../hrFormClasses';
...
import { AlertCircle, CheckCircle2, ChevronLeft, ChevronRight, ClipboardList, Download, Eye, FileText, Search, X } from 'lucide-react';
import { cn } from '@/lib/utils';
import { Label } from '@/components/ui/label';
const hrCanDecideOnStatus = status => ['submitted', 'under_review'].includes(status);
...
/** Giờ trong form là giờ VN (cùng ý nghĩa với dòng ghi chú), không phụ thuộc múi giờ trình duyệt HR. */
...
function buildInterviewInviteNotes(form) {
  const f = form || {};
  const lines = [];
  const fmt = INTERVIEW_FORMAT_LABELS[f.interviewFormat] || f.interviewFormat || 'Không xác định';
...
/** HR chỉ báo kết quả PV khi ứng viên đã xác nhận lịch và thời điểm PV đã qua. */
function canReportInterviewOutcome(application) {
  if (!application || application.status !== 'interview_confirmed') return false;
...
const HRApplicationManagement = () => {
  ...
  const [statusReasonModal, setStatusReasonModal] = useState(() => emptyStatusReasonModal());
  const [interviewOutcomeModal, setInterviewOutcomeModal] = useState(() => emptyInterviewOutcomeModal());
  const prevListPageRef = useRef(1);
  const jobFilterParam = jobIdFromUrl || undefined;
  const hasCoverLetterText = text => typeof text === 'string' && text.trim().length > 0;
  useEffect(() => {
    if (!jobIdFromUrl) {
      setPageType('all');
      setCurrentJobTitle('');
    ...
    (async () => {
      try {
        const res = await makeJsonRequest(`/api/hr/jobs/${jobIdFromUrl}`);
  ...
  }, [jobFilterParam, statusFilter, searchTerm, sortBy, sortOrder, initialLoad]);
  useEffect(() => {
    if (!initialLoad) {
      const cameBackToFirstPage = prevListPageRef.current > 1 && currentPage === 1;
  ...
  }, [currentPage, jobFilterParam, statusFilter, searchTerm, sortBy, sortOrder, initialLoad, fetchApplications]);
  ...
  const normalizeFeedbackFromApplication = app => {
    if (!app) return null;
    const rawCandidates = [app.feedback, app.existingFeedback, app.interviewerFeedback, app.latestFeedback, app.interview?.feedback, app.interview?.existingFeedback, app.latestInterview?.feedback, app.latestInterview?.existingFeedback, ...(Array.isArray(app.interviews) ? app.interviews.map(iv => iv?.feedback || iv?.existingFeedback).filter(Boolean) : [])].filter(Boolean);
    const scoreFeedback = f => {
      if (!f || typeof f !== 'object') return 0;
      let score = 0;
      const fields = ['overallRating', 'technicalSkills', 'problemSolving', 'candidateExperienceRating', 'recommendation', 'additionalNotes', 'submittedAt'];
      fields.forEach(k => {
  ...
  const handleApplicationAction = async (action, applicationId) => {
    switch (action) {
      case 'view':
        const application = applications.find(app => (app.id || app._id) === applicationId || String(app.id ?? app._id) === String(applicationId));
  ...
  const handleStatusChange = async (applicationId, newStatus, notes = '', scheduleMeta = {}) => {
    try {
      const result = await updateApplicationStatus(applicationId, newStatus, notes, scheduleMeta);
      if (result.success) {
        const filterWouldExclude = statusFilter !== 'all' && newStatus !== statusFilter;
        ...
        await fetchApplications({
          ...
          limit: 20,
          job: jobFilterParam,
          status: filterWouldExclude ? undefined : statusFilter !== 'all' ? statusFilter : undefined,
          search: searchTerm || undefined,
          sortBy,
          sortOrder
        });
  ...
  const handleConfirmStatusReason = async () => {
    ...
    const res = await handleStatusChange(statusReasonModal.applicationId, statusReasonModal.status, reason, scheduleMeta);
    if (res?.success) {
  ...
  const handleConfirmInterviewOutcome = async () => {
    ...
    const res = await handleStatusChange(interviewOutcomeModal.applicationId, newStatus, trimmed);
    if (res?.success) {
  ...
  const exportApplicationsToExcel = () => {
    ...
    const sanitizeSheetName = name => {
      const safe = String(name || 'Không xác định').replace(/[\\/*?:[\]]/g, ' ').trim();
    ...
    Object.entries(byJob).forEach(([jobTitle, jobApps]) => {
      ...
      XLSX.utils.book_append_sheet(workbook, sheet, sanitizeSheetName(jobTitle));
    });
    XLSX.writeFile(workbook, `danh-sach-ung-vien-theo-job-${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const targets = applications.filter(a => a && ['interview_scheduled', 'interview_confirmed'].includes(a.status));
      ...
      await Promise.all(needFetch.map(async a => {
        const id = a.id;
        const urls = [`/api/hr/interviews?applicationId=${id}&limit=3`, `/api/hr/interviews?application=${id}&limit=3`, `/api/hr/interviews?appId=${id}&limit=3`];
        let found = false;
        for (const u of urls) {
          try {
  ...
  const hasFeedbackData = !!(appFeedback && (appFeedback.overallRating != null || !!appFeedback.recommendation || Array.isArray(appFeedback.strengths) && appFeedback.strengths.length > 0 || Array.isArray(appFeedback.weaknesses) && appFeedback.weaknesses.length > 0 || !!appFeedback.additionalNotes || !!appFeedback.submittedAt));
  useEffect(() => {
    let cancelled = false;
    const fetchFeedback = async () => {
      if (!showApplicationModal || !selectedApplication) return;
      const hasInline = inlineFeedback && (inlineFeedback.overallRating != null || inlineFeedback.recommendation || inlineFeedback.strengths?.length || inlineFeedback.weaknesses?.length);
      if (hasInline) {
      ...
      setExternalFeedback(null);
      const candidates = [];
      const tryUrls = [`/api/hr/interviews?applicationId=${selectedApplication.id}&limit=5`, `/api/hr/interviews?application=${selectedApplication.id}&limit=5`, `/api/hr/interviews?appId=${selectedApplication.id}&limit=5`];
      ...
      if (!cancelled && candidates.length) {
        ...
        if (withFb.length) {
          const pickRichest = arr => arr.sort((a, b) => {
            const score = f => {
              let s = 0;
              ['overallRating', 'technicalSkills', 'problemSolving', 'candidateExperienceRating', 'recommendation', 'additionalNotes', 'submittedAt'].forEach(k => {
                if (f?.[k] != null && f?.[k] !== '') s++;
              });
  ...
  return <HRLayout>
      <div className={HR_PAGE}>
        <div className={HR_PAGE_HEADER}>
            <div className="min-w-0 flex-1">
              <h1 className={HR_H1}>{pageType === 'filtered' ? 'Hồ sơ theo tin tuyển dụng' : 'Hồ sơ ứng viên'}</h1>
              {pageType === 'filtered' && currentJobTitle && (
                <h2 className="mt-1 font-['Open_Sans'] text-lg font-semibold text-foreground sm:text-xl">{currentJobTitle}</h2>
              )}
              <p className={HR_SUBTITLE}>
                {pageType === 'filtered' ? 'Danh sách hồ sơ cho vị trí đang xem' : 'Tiếp nhận, đánh giá và cập nhật tiến độ ứng viên trên findme'}
              </p>
              {pageType === 'filtered' && (
                <div className="mt-2">
                  <Button variant="link" className="h-auto p-0 font-['Roboto'] text-primary" asChild>
                    <Link to="/hr/applications">← Xem toàn bộ hồ sơ</Link>
                  </Button>
                </div>
              )}
            </div>
        ...
        <Card className="mb-4 shadow-sm sm:mb-6">
          <CardContent className="space-y-4 pt-6">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div className="md:col-span-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className={cn(HR_NATIVE_FIELD, "min-h-11 pl-9 pr-24 font-['Roboto']")}
              ...
              <div>
                ...
                >
                  ...
                  <option value="interview_confirmed">Đã xác nhận lịch phỏng vấn</option>
            ...
            <span className="shrink-0 font-['Roboto'] text-sm font-medium text-muted-foreground">Sắp xếp:</span>
            <select
              value={sortBy}
            ...
            >
              <svg className={`size-4 ${sortOrder === 'asc' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
        ...
        {error && (
          <div className="mb-6 space-y-3">
            <Alert variant="destructive">
              ...
              <AlertDescription className="font-['Roboto'] text-destructive/90">{error}</AlertDescription>
            </Alert>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="font-['Roboto'] text-xs">Vị trí tuyển dụng</TableHead>
                    <TableHead className="font-['Roboto'] text-xs">Ngày nộp</TableHead>
                    <TableHead className="font-['Roboto'] text-xs">Điểm hồ sơ</TableHead>
                    <TableHead className="font-['Roboto'] text-xs">Kỹ năng</TableHead>
                ...
                <TableBody>
                  {applications.map((application, rowIndex) => {
                    ...
                    const hasFeedbackHint = !!(inlineFb || feedbackByAppId[appRowId]);
                    const useProcessCompleteBadge =
                      hasFeedbackHint && ['interview_scheduled', 'interview_confirmed'].includes(application.status);
                    const rowStatusLabel = useProcessCompleteBadge
                      ? formatStatus('completed')
                      : getInterviewPassFailLabel(application) ?? formatStatus(application.status);
                    const rowBadgeKey = useProcessCompleteBadge
                      ? 'completed'
                      : getInterviewPassFailBadgeKey(application) ?? application.status;
                    const stt = (currentPage - 1) * (pagination.limit || 20) + rowIndex + 1;
                    return (
                      <TableRow key={appRowId}>
                        <TableCell className="text-center font-['Roboto'] text-sm tabular-nums text-muted-foreground">
                          {stt}
                        </TableCell>
                        <TableCell className="align-top font-mono text-xs font-semibold text-foreground" title={recruitmentJobIdRaw(application.job) || undefined}>
                          {recruitmentJobIdRaw(application.job) || '—'}
                        </TableCell>
                        <TableCell>
                          <div className="min-w-0">
                            <div className="font-['Open_Sans'] text-sm font-medium text-foreground">{application.candidate.name}</div>
                            <div className="font-['Roboto'] text-sm text-muted-foreground">{application.experience}</div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="font-['Roboto'] text-sm text-foreground">{application.job.title}</div>
                          <div className="font-['Roboto'] text-sm text-muted-foreground">{application.job.department}</div>
                        </TableCell>
                        <TableCell className="font-['Roboto'] text-sm text-muted-foreground">
                          {formatDateVN(application.appliedDate)}
                        </TableCell>
                        <TableCell>
                          <div className={cn("font-['Roboto'] text-sm", hrScoreTextClass(application.resumeScore))}>
                            {application.resumeScore != null && application.resumeScore !== '' ? `${application.resumeScore}/10` : '—'}
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <Badge variant="outline" className={cn('font-normal', hrStatusBadgeClass(rowBadgeKey))}>
                            {rowStatusLabel}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            <Button
                              ...
                              size="icon"
                              className="size-8"
                              onClick={() => handleApplicationAction('view', appRowId)}
                              title="Xem chi tiết"
                            >
                            ...
                            {hasCoverLetterText(application.coverLetter) && (
                              <Button
                                ...
                                onClick={e => openCoverLetterModal(application, e)}
                            ...
                            {canReportInterviewOutcome(application) && (
                              <Button
                                ...
                                size="icon"
                                className="size-8 text-primary"
                                onClick={() => handleApplicationAction('interview_outcome', appRowId)}
                            ...
                            {hrCanDecideOnStatus(application.status) && (
                              <>
                                <Button
                                  ...
                                  size="icon"
                                  className="size-8 text-primary"
                                  onClick={() => handleApplicationAction('approve', appRowId)}
                      title="Mời phỏng vấn"
                                >
                                ...
                                <Button
                                  ...
                                  size="icon"
                                  className="size-8 text-primary"
                                  onClick={() => handleApplicationAction('reject', appRowId)}
                      title="Từ chối hồ sơ"
                                >
        ...
        {!loading && applications.length === 0 && (
          <Card className="shadow-sm">
            <CardContent className="py-12 text-center">
              <svg className="mx-auto size-12 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
              </svg>
              <h3 className="mt-2 font-['Open_Sans'] text-sm font-medium text-foreground">Không tìm thấy hồ sơ phù hợp</h3>
              <p className="mt-1 font-['Roboto'] text-sm text-muted-foreground">Không có hồ sơ nào khớp bộ lọc hiện tại.</p>
            </CardContent>
          </Card>
        )}
      </div>
      ...
      <HRModal
        ...
        header={
          selectedApplication ? (
            <div>
              <h3 className="font-['Open_Sans'] text-2xl font-semibold text-foreground">{selectedApplication.candidate.name}</h3>
              {recruitmentJobIdRaw(selectedApplication.job) ? (
                <p className="mt-1 font-mono text-sm text-muted-foreground" title={recruitmentJobIdRaw(selectedApplication.job)}>
                  {recruitmentJobIdRaw(selectedApplication.job)}
                </p>
              ) : null}
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <Badge
                  ...
                  className={cn(
                    'font-normal',
                    hrStatusBadgeClass(getInterviewPassFailBadgeKey(selectedApplication) ?? selectedApplication.status)
                  )}
                >
                  {getInterviewPassFailLabel(selectedApplication) ?? formatStatus(selectedApplication.status)}
                </Badge>
                <span className="font-['Roboto'] text-sm text-muted-foreground">
                  Nộp ngày {formatDateVN(selectedApplication.appliedDate || selectedApplication.createdAt)}
                </span>
              </div>
            </div>
          ) : null
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <Button
                  ...
                  title={!selectedApplication.resumeUrl ? 'Không có hồ sơ' : 'Xem hồ sơ ứng viên'}
                  onClick={async () => {
                    try {
              ...
              <div className="flex flex-wrap justify-end gap-3">
                <Button variant="outline" className="font-['Roboto']" onClick={() => setShowApplicationModal(false)}>
                  Đóng
                </Button>
                {hrCanDecideOnStatus(selectedApplication.status) && (
                  <>
                    <Button
                      ...
                      onClick={() => {
                        handleApplicationAction('reject', selectedApplication.id || selectedApplication._id);
                    ...
                    <Button
                      className="font-['Roboto']"
                      onClick={() => {
                        handleApplicationAction('approve', selectedApplication.id || selectedApplication._id);
                        setShowApplicationModal(false);
                      }}
                    >
                      Mời phỏng vấn
                ...
                {canReportInterviewOutcome(selectedApplication) && (
                  <Button
                    className="font-['Roboto']"
                    onClick={() => {
                      handleApplicationAction('interview_outcome', selectedApplication.id || selectedApplication._id);
                      setShowApplicationModal(false);
                    }}
                  >
                ...
                {selectedApplication.status === 'rejected' && (
                  <Alert variant="destructive" className="py-2">
                    <AlertDescription className="font-['Roboto']">Đơn đã bị từ chối</AlertDescription>
                  </Alert>
                )}
              </div>
            </div>
        ...
        {selectedApplication && (
          <>
            <div className="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
              <div>
                <h4 className="mb-2 font-['Roboto'] text-sm font-medium text-muted-foreground">Thông tin liên hệ</h4>
                <p className="font-['Roboto'] text-foreground">{selectedApplication.candidate.email}</p>
                <p className="font-['Roboto'] text-foreground">
                  {selectedApplication.candidate.phone || 'Không có số điện thoại'}
                </p>
              </div>
              <div>
                <h4 className="mb-2 font-['Roboto'] text-sm font-medium text-muted-foreground">Vị trí ứng tuyển</h4>
                <p className="font-['Roboto'] text-foreground">{selectedApplication.job.title}</p>
                <p className="font-['Roboto'] text-muted-foreground">{selectedApplication.job.department}</p>
              </div>
              <div>
                <h4 className="mb-2 font-['Roboto'] text-sm font-medium text-muted-foreground">Kinh nghiệm</h4>
                <p className="font-['Roboto'] text-foreground">{selectedApplication.experience}</p>
              </div>
              <div>
                <h4 className="mb-2 font-['Roboto'] text-sm font-medium text-muted-foreground">Điểm hồ sơ</h4>
                <p className={cn("font-['Roboto'] text-lg", hrScoreTextClass(selectedApplication.resumeScore))}>
                  {selectedApplication.resumeScore != null && selectedApplication.resumeScore !== '' ? `${selectedApplication.resumeScore}/10` : '—'}
                </p>
              </div>
            </div>

            <div className="mb-6">
              <h4 className="mb-2 font-['Roboto'] text-sm font-medium text-muted-foreground">Kỹ năng</h4>
              <div className="flex flex-wrap gap-2">
                {selectedApplication.skills && selectedApplication.skills.length > 0 ? (
                  ...
                  <span className="font-['Roboto'] text-sm text-muted-foreground">Không có thông tin kỹ năng</span>
                )}
              </div>
            </div>
              ...
              <Card className="mb-6 border-dashed bg-muted/30 shadow-none">
                <CardContent className="pt-6">
                  <div className="mb-2 flex items-center justify-between gap-2">
                    <h4 className="font-['Roboto'] text-sm font-medium text-foreground">Thư giới thiệu</h4>
                    <Button variant="link" className="h-auto p-0 font-['Roboto'] text-xs" type="button" onClick={() => openCoverLetterModal(selectedApplication)}>
                      Mở rộng
                    </Button>
                  </div>
            ...
            <div className="mb-6 border-t border-border pt-6">
              <h4 className="mb-4 font-['Open_Sans'] text-lg font-medium text-foreground">Phân tích hồ sơ tự động</h4>
              {selectedApplication.aiAnalysis ? (
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                  <Card className="shadow-sm">
                    <CardContent className="pt-6">
                      <p className="mb-1 font-['Roboto'] text-xs font-medium text-muted-foreground">Mức độ phù hợp kỹ năng</p>
                      <p className="font-['Open_Sans'] text-lg font-semibold text-foreground">
                        {Math.round(selectedApplication.aiAnalysis.skillsMatch || 0)}%
                      </p>
                    </CardContent>
                  </Card>
                  <Card className="shadow-sm">
                    <CardContent className="pt-6">
                      <p className="mb-1 font-['Roboto'] text-xs font-medium text-muted-foreground">Mức độ phù hợp kinh nghiệm</p>
                      <p className="font-['Open_Sans'] text-lg font-semibold text-foreground">
                        {Math.round(selectedApplication.aiAnalysis.experienceMatch || 0)}%
                      </p>
                    </CardContent>
                  </Card>
                  <Card className="shadow-sm">
                    <CardContent className="pt-6">
                      <p className="mb-1 font-['Roboto'] text-xs font-medium text-muted-foreground">Mức độ phù hợp tổng thể</p>
                      <p className="font-['Open_Sans'] text-lg font-semibold text-foreground">
                        {Math.round(selectedApplication.aiAnalysis.overallFit || 0)}%
                      </p>
                    </CardContent>
                  </Card>
                </div>
              ) : (
                <p className="font-['Roboto'] text-sm text-muted-foreground">Chưa có kết quả phân tích tự động cho hồ sơ này.</p>
              )}

              {selectedApplication.aiAnalysis && (
                <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div>
                    <h5 className="mb-2 font-['Open_Sans'] text-sm font-medium text-foreground">Điểm mạnh chính</h5>
                    {selectedApplication.aiAnalysis.strengths && selectedApplication.aiAnalysis.strengths.length > 0 ? (
                      <ul className="list-disc space-y-1 pl-4 font-['Roboto'] text-sm text-muted-foreground">
                        {selectedApplication.aiAnalysis.strengths.map((s, idx) => (
                          <li key={idx}>{s}</li>
                        ))}
                      </ul>
                    ) : (
                      <p className="font-['Roboto'] text-sm text-muted-foreground">Chưa phát hiện điểm mạnh nào.</p>
                    )}
                  </div>
                  <div>
                    <h5 className="mb-2 font-['Open_Sans'] text-sm font-medium text-foreground">Điểm cần lưu ý</h5>
                    {selectedApplication.aiAnalysis.concerns && selectedApplication.aiAnalysis.concerns.length > 0 ? (
                      <ul className="list-disc space-y-1 pl-4 font-['Roboto'] text-sm text-muted-foreground">
                        {selectedApplication.aiAnalysis.concerns.map((c, idx) => (
                          <li key={idx}>{c}</li>
                        ))}
                      </ul>
                    ) : (
                      <p className="font-['Roboto'] text-sm text-muted-foreground">Chưa phát hiện vấn đề lớn nào.</p>
                    )}
                  </div>
                </div>
              )}
      ...
      <HRModal
        ...
        footer={
          <div className="flex justify-end gap-2">
            <Button variant="secondary" disabled={interviewOutcomeModal.submitting} onClick={() => setInterviewOutcomeModal(emptyInterviewOutcomeModal())}>
              Hủy
            </Button>
            <Button disabled={interviewOutcomeModal.submitting} onClick={handleConfirmInterviewOutcome}>
              {interviewOutcomeModal.submitting ? 'Đang gửi...' : 'Gửi cho ứng viên'}
            </Button>
          </div>
        }
      >
        {interviewOutcomeModal.open && (
          <div className="space-y-4">
            <p className="font-['Roboto'] text-sm text-muted-foreground">
              Chọn kết quả và nhập nhận xét — hệ thống sẽ tự động gửi thư chúc mừng nhận việc (nếu đạt) hoặc thư từ chối (nếu không đạt) trực tiếp tới email ứng viên, đồng thời cập nhật thông báo trên hệ thống.
            </p>
            <div>
              <span className="mb-2 block font-['Roboto'] text-sm font-medium text-foreground">Kết quả</span>
              <div className="flex flex-wrap gap-4 font-['Roboto'] text-sm">
                <label className="flex cursor-pointer items-center gap-2">
                  <input
                    ...
                    onChange={() => setInterviewOutcomeModal(prev => ({
                  ...prev,
                  outcome: 'passed'
                }))}
                  />
            ...
            <div>
              <Label className="mb-1.5 block font-['Roboto'] text-foreground">
                Nhận xét / lý do gửi ứng viên <span className="text-destructive">*</span>
              </Label>
              <textarea
                value={interviewOutcomeModal.reason}
                disabled={interviewOutcomeModal.submitting}
                onChange={e => setInterviewOutcomeModal(prev => ({
                  ...prev,
      ...
      <HRModal
        ...
        size="lg"
        title={statusReasonModal.status === 'interview_scheduled' ? 'Thông báo mời phỏng vấn' : 'Lý do từ chối hồ sơ'}
        footer={
          <div className="flex justify-end gap-2">
            <Button variant="secondary" disabled={statusReasonModal.submitting} onClick={() => setStatusReasonModal(emptyStatusReasonModal())}>
              Hủy
            </Button>
            <Button disabled={statusReasonModal.submitting} onClick={handleConfirmStatusReason}>
              {statusReasonModal.submitting ? 'Đang gửi...' : 'Xác nhận'}
            </Button>
          </div>
        }
      >
        {statusReasonModal.open && <div>
              <p className="mb-4 font-['Roboto'] text-sm text-muted-foreground">
                {statusReasonModal.status === 'interview_scheduled' ? 'Thông tin dưới đây được ghép thành nội dung kèm email mời phỏng vấn gửi tới ứng viên.' : 'Nội dung này được gửi kèm thông báo tới ứng viên khi từ chối hồ sơ.'}
              </p>
              {statusReasonModal.status === 'interview_scheduled' ? <div className="space-y-4">
                  <div>
                    <Label className="mb-1.5 block font-['Roboto'] text-foreground">
                      Hình thức phỏng vấn <span className="text-destructive">*</span>
                    </Label>
                    <select value={statusReasonModal.inviteForm?.interviewFormat ?? 'online'} onChange={e => setStatusReasonModal(prev => ({
                ...prev,
                inviteForm: {
                  ...prev.inviteForm,
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <input type="date" min={interviewInviteMinDateYyyyMmDd()} value={statusReasonModal.inviteForm?.expectedDate ?? ''} onChange={e => setStatusReasonModal(prev => ({
                  ...prev,
                  inviteForm: {
                    ...prev.inviteForm,
                    ...
                    <div>
                      ...
                      <input type="time" value={statusReasonModal.inviteForm?.expectedTime ?? ''} onChange={e => setStatusReasonModal(prev => ({
                  ...prev,
                  inviteForm: {
                    ...prev.inviteForm,
                  ...
                  <div>
                    <Label className="mb-1.5 block font-['Roboto'] text-foreground">
                      Địa điểm hoặc link họp <span className="text-destructive">*</span>
                    </Label>
                    <input type="text" value={statusReasonModal.inviteForm?.venueOrLink ?? ''} onChange={e => setStatusReasonModal(prev => ({
                ...prev,
                inviteForm: {
                  ...prev.inviteForm,
                  venueOrLink: e.target.value
                }
              }))} placeholder="Liên kết họp trực tuyến hoặc địa chỉ cụ thể" className={HR_INPUT} />
                  </div>
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                      <Label className="mb-1.5 block font-['Roboto'] text-foreground">Người liên hệ</Label>
                      <input type="text" value={statusReasonModal.inviteForm?.contactPerson ?? ''} onChange={e => setStatusReasonModal(prev => ({
                  ...prev,
                  inviteForm: {
                    ...prev.inviteForm,
                    contactPerson: e.target.value
                  }
                }))} placeholder="Họ tên người phụ trách" className={HR_INPUT} />
                    </div>
                    <div>
                      <Label className="mb-1.5 block font-['Roboto'] text-foreground">Số điện thoại hoặc thư điện tử liên hệ</Label>
                      <input type="text" value={statusReasonModal.inviteForm?.contactInfo ?? ''} onChange={e => setStatusReasonModal(prev => ({
                  ...prev,
                  inviteForm: {
                    ...prev.inviteForm,
                    contactInfo: e.target.value
                  }
                }))} placeholder="Số điện thoại hoặc thư điện tử" className={HR_INPUT} />
                    </div>
                  </div>
                  <div>
                    <Label className="mb-1.5 block font-['Roboto'] text-foreground">Ghi chú thêm</Label>
                    <textarea value={statusReasonModal.inviteForm?.additionalNotes ?? ''} onChange={e => setStatusReasonModal(prev => ({
                ...prev,
                inviteForm: {
                  ...prev.inviteForm,
                  additionalNotes: e.target.value
                }
              }))} rows={3} placeholder="Thông tin bổ sung gửi kèm ứng viên nếu cần" className={cn(HR_TEXTAREA, 'min-h-[80px] resize-y')} />
                  </div>
                </div> : <textarea value={statusReasonModal.reason} onChange={e => setStatusReasonModal(prev => ({
            ...prev,
            reason: e.target.value
          }))} rows={5} placeholder="Nhập lý do từ chối..." className={HR_TEXTAREA} />}
            </div>}
      </HRModal>

      <HRModal
        open={coverLetterModal.open}
        onClose={() => setCoverLetterModal({ open: false, text: '', candidateName: '' })}
        size="lg"
        title="Thư giới thiệu"
        subtitle={coverLetterModal.candidateName || undefined}
        footer={<Button type="button" variant="secondary" className="font-['Roboto']" onClick={() => setCoverLetterModal({ open: false, text: '', candidateName: '' })}>
              Đóng
            </Button>}
      >
        {coverLetterModal.open && <p className="whitespace-pre-wrap font-['Roboto'] text-sm leading-relaxed text-foreground">
