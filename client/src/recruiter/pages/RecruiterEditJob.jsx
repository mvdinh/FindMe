...
import { HR_INPUT_PILL, HR_TEXTAREA_PILL } from '../recruiterFormClasses';
...
const RecruiterEditJob = () => {
  ...
  const handleSubmit = async (e, actionType = 'save') => {
    ...
    try {
      ...
      const response = await makeJsonRequest(`/api/recruiter/jobs/${jobId}`, {
        method: 'PUT',
        headers: {
      ...
      <div className={HR_PAGE}>
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline" className={cn("rounded-full font-['Roboto']", recruiterStatusBadgeClass(jobData.status))}>
                {jobStatusLabels[jobData.status] || jobData.status}
              </Badge>
            </div>
          </div>
          <Button type="button" variant="outline" className="font-['Roboto'] shrink-0" onClick={() => navigate('/recruiter/jobs')} disabled={formBusy}>
        ...
        <form onSubmit={handleSubmit} className="space-y-8" aria-busy={formBusy}>
          ...
          <div className="flex flex-col gap-4 border-t border-border pt-6 sm:flex-row sm:items-center sm:justify-between">
            <Button type="button" variant="outline" className="w-full font-['Roboto'] sm:w-auto" onClick={() => navigate('/recruiter/jobs')}>
              Hủy
            </Button>
