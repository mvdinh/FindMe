...
async function extractTextFromPdfBuffer(buffer) {
  ...
  try {
    ...
    const numpages = typeof textResult?.total === 'number' ? textResult.total : Array.isArray(textResult?.pages) ? textResult.pages.length : undefined;
    return {
      text,
      numpages
    };
...
function parseScheduledAtFromHrNote(note) {
  ...
  const m = note.match(/Thời gian dự kiến:\s*(\d{4}-\d{2}-\d{2})\s+lúc\s+(\d{1,2}):(\d{2})(?::(\d{2}))?/);
  if (!m) return null;
...
async function getApplicantActiveResume(applicantId) {
  const user = await User.findById(applicantId).select('currentResumeId profile.currentResumeId');
  if (!user) return null;
  const candidates = [user.profile?.currentResumeId, user.currentResumeId].filter(Boolean);
...
router.post('/', auth, upload.single('customResume'), async (req, res) => {
  try {
    ...
    const useProfileResumeRequested = String(useProfileResume).toLowerCase() === 'true';
    ...
    const normalizedExperience = experienceInput ? experienceAliases[experienceInput] || experienceInput : 'fresher';
    ...
    const applicantAtsPoliteMessage = jobTitle => `Chúng tôi đánh giá cao kỹ năng chuyên môn của bạn, nhưng bạn chưa phù hợp với vị trí ${jobTitle}.`;
    ...
    const resumeFromProfile = !req.file && useProfileResumeRequested ? await getApplicantActiveResume(applicantId) : null;
    ...
    try {
      ...
      let resumeSource = req.file && req.file.buffer ? {
        buffer: req.file.buffer,
        mimeType: req.file.mimetype || '',
        fileName: req.file.originalname || ''
      } : resumeFromProfile ? {
        buffer: resumeFromProfile.fileData ? Buffer.from(resumeFromProfile.fileData, 'base64') : null,
        mimeType: resumeFromProfile.mimeType || 'application/pdf',
        fileName: resumeFromProfile.originalName || resumeFromProfile.fileName || 'resume'
      } : null;
      ...
      if (resumeSource && resumeSource.buffer) {
        ...
        if (isPdf && pdfParseModule) {
          ...
          try {
            ...
            console.warn('pdf-parse failed to extract resume text:', e.message || e);
            resumeText = '';
          }
        } else if (!isPdf) {
          ...
          try {
            ...
            console.warn('Failed to decode resume buffer as utf8:', e.message || e);
            resumeText = '';
          }
        }
        if (isPdf && atsEngineResolved === 'scan_cv' && scanUrl && !(resumeText || '').trim()) {
          try {
            console.info('[ATS] PDF text empty — calling scan_chamdiemCV /api/cv/upload then /api/cv/', {
            ...
            const row = Array.isArray(cvs) ? cvs.find(c => c.id === up.cv_id) : null;
            ...
            if (t.trim()) {
              ...
              extractionMeta.wordCount = t.trim().split(/\s+/).filter(Boolean).length;
              extractionMeta.warnings = extractionMeta.warnings.filter(
                w => w !== 'PDF_TEXT_EXTRACTION_EMPTY' && w !== 'PDF_TEXT_EXTRACTION_FAILED'
              );
            }
          } catch (e) {
            extractionMeta.warnings.push('SCAN_CHAMDIEMCV_PDF_EXTRACT_FAILED');
            console.warn('scan_chamdiemCV PDF extract fallback failed:', e?.message || e);
          }
        }
      }
      const geminiArgs = {
      ...
      const docMeta = {
        fileSize: application.useProfileResume ? resumeFromProfile?.fileSize : application.customResume?.fileSize,
        fileName: application.useProfileResume ? resumeFromProfile?.originalName || resumeFromProfile?.fileName : application.customResume?.fileName,
        fileType: application.useProfileResume ? resumeFromProfile?.mimeType : application.customResume?.fileMimeType,
      ...
      if (wantsScanCv && !canUseScan) {
        console.warn('[ATS] scan_cv selected but scan_chamdiemCV /api/analyze/match will NOT run — fix env, job text, or resume text.', {
          jobId: jobId?.toString?.(),
      ...
      if (canUseScan) {
        try {
          console.info('[ATS] invoking scan_chamdiemCV POST /api/analyze/match', { base: scanUrl });
          ...
          console.error('ATS scan_cv failed, falling back to Gemini:', scanErr?.message || scanErr);
          ...
          application.aiAnalysis = {
            ...
            validation: {
              ...validationBlock,
              warnings: [...validationBlock.warnings, 'CV_MATCHING_HTTP_ERROR_FALLBACK_GEMINI']
            }
          };
        }
      } else {
        ...
        application.aiAnalysis = {
          ...
          overallScore: typeof aiResult.overallScore === 'number' ? aiResult.overallScore : 50,
          keyStrengths: Array.isArray(aiResult.keyStrengths) ? aiResult.keyStrengths : [],
          potentialConcerns: [...extraConcerns, ...(Array.isArray(aiResult.potentialConcerns) ? aiResult.potentialConcerns : [])],
          recommendedQuestions: Array.isArray(aiResult.recommendedQuestions) ? aiResult.recommendedQuestions : [],
          extractedInfo: aiResult.extractedInfo || {},
      ...
      const atsThreshold = typeof job?.atsResumeThreshold === 'number' ? Math.min(100, Math.max(0, job.atsResumeThreshold)) : 60;
      ...
      if (atsEnabled && !skipAtsBecauseCoverLetter && typeof resumeScore === 'number' && resumeScore < atsThreshold) {
      ...
      } else if (atsEnabled && !skipAtsBecauseCoverLetter && typeof resumeScore === 'number' && resumeScore >= atsThreshold) {
        hrAtsPassInfo = {
          score: resumeScore,
      ...
      console.error('AI resume analysis failed for application', application._id?.toString?.(), aiError);
    }
    ...
    try {
      const job = await Job.findById(jobId).select('title postedBy atsEnabled atsResumeThreshold atsSkipWhenCoverLetter');
      if (job) {
        const hrPassAts = hrAtsPassInfo && application.status !== 'rejected';
        const hrTitle = hrPassAts ? 'Ứng viên đạt ngưỡng ATS' : 'Có đơn ứng tuyển mới';
        const hrMessage = hrPassAts ? `${firstName} ${lastName} đã ứng tuyển «${job.title}». Điểm CV ${hrAtsPassInfo.score}% đạt ngưỡng ATS (${hrAtsPassInfo.threshold}%). Vui lòng xem xét và xử lý tuyển dụng trực tiếp trên hệ thống.` : `${firstName} ${lastName} đã ứng tuyển vào ${job.title}`;
        await createAndEmit({
        ...
        await createAndEmit({
          ...
          toRole: 'applicant',
          type: 'application_submitted',
          title: application.status === 'rejected' ? 'Cập nhật đơn ứng tuyển' : 'Nộp đơn thành công',
          message: application.status === 'rejected' ? applicantAtsPoliteMessage(job.title) : `Đơn ứng tuyển của bạn cho vị trí ${job.title} đã được gửi thành công và đang chờ nhà tuyển dụng xem xét.`,
        ...
        if (application.status === 'rejected') {
          ...
          const threshold = typeof job.atsResumeThreshold === 'number' ? job.atsResumeThreshold : 60;
          await createAndEmit({
            toUserId: job.postedBy,
            toRole: 'hr',
            type: 'application_status_changed',
            title: 'ATS tự động từ chối ứng viên',
            message: `${firstName} ${lastName} bị ATS từ chối cho vị trí ${job.title}${typeof score === 'number' ? ` (điểm CV ${score}% < ngưỡng ${threshold}%)` : ''}.`,
            actionUrl: `/hr/applications/${application._id}`,
            entity: {
              kind: 'Application',
              id: application._id
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    if (status && typeof status === 'string') {
      const allowed = ['submitted', 'under_review', 'interview_scheduled', 'interview_confirmed', 'interview_passed', 'offer_extended', 'offer_accepted', 'offer_declined', 'rejected', 'withdrawn'];
    ...
    const applications = await Application.find(query).populate([{
      path: 'job',
      select: 'title location jobType salaryRange status jobCode code referenceCode'
    }, {
...
router.post('/confirm-interview', auth, [body('applicationId').isMongoId().withMessage('applicationId không hợp lệ')], async (req, res) => {
  try {
    ...
    try {
      const job = await Job.findById(application.job).select('title postedBy');
      if (job) {
        await createAndEmit({
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    if (['interview_passed', 'offered', 'hired', 'withdrawn'].includes(application.status)) {
      return res.status(400).json({
