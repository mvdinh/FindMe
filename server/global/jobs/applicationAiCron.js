...
async function runAtsAiPipeline({ applicationId }) {
  ...
  try {
    ...
    if (resumeBuffer) {
      const buffer = Buffer.isBuffer(resumeBuffer)
        ? resumeBuffer
        : Buffer.from(resumeBuffer);
      ...
      aiAnalysis = {
        resumeScore:
          typeof aiResult.overallScore === "number"
            ...
            : 50,
        skillsMatch:
          typeof aiResult.skillsMatch === "number" ? aiResult.skillsMatch : 50,
        experienceMatch:
        ...
        documentMetadata: docMeta,
        validation:
          wantsScanCv && !canUseScan
            ? {
                ...validationBlock,
                warnings: [
                  ...validationBlock.warnings,
                  "SCAN_CV_SKIPPED_USE_GEMINI",
                ],
              }
            : validationBlock,
      };
    ...
    await Application.findByIdAndUpdate(applicationId, {
      $set: {
        ...
        "aiProcessing.finishedAt": new Date(),
        "aiProcessing.error": String(
          err?.message || err || "AI_PIPELINE_ERROR",
        ).slice(0, 500),
      },
    });
  }
