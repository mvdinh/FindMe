...
router.get('/', [query('page').optional().isInt({
  ...
  min: 1,
  max: 100
}), query('status').optional().isIn(['all', 'unread', 'read'])], async (req, res) => {
  try {
    ...
    const [items, total] = await Promise.all([Notification.find(filter).sort({
      createdAt: -1
    }).skip(skip).limit(parseInt(limit)), Notification.countDocuments(filter)]);
    return res.json({
      success: true,
      data: {
        items,
...
router.delete('/', async (req, res) => {
  try {
    ...
    if (invalid) return res.status(400).json({
      success: false,
      message: 'Invalid id in ids array'
    });
    const baseFilter = {
      $or: [{
        user: req.user.id
