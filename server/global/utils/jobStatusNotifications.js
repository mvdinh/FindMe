...
async function notifyActorAndPeerRoleOnStatusChange({
  ...
  if (ar !== 'hr' && ar !== 'admin') return;
  try {
    const actor = await User.findById(actorUserId).select('firstName lastName').lean();
    const actorName = actor ? `${actor.firstName || ''} ${actor.lastName || ''}`.trim() || 'Người dùng' : 'Người dùng';
    const newL = statusLabel(newStatus);
    const selfPath = ar === 'admin' ? '/admin/jobs' : '/hr/jobs';
    await createAndEmit({
    ...
    const peerRole = ar === 'hr' ? 'admin' : 'hr';
    const peerTitle = ar === 'hr' ? 'HR cập nhật trạng thái tin' : 'Admin cập nhật trạng thái tin';
    const peerUrl = peerRole === 'admin' ? '/admin/jobs' : '/hr/jobs';
