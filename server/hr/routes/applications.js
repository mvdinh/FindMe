...
router.get('/', auth, authorize('hr', 'admin'), [query('page').optional().isInt({
  min: 1
...
router.put('/:id/status', auth, authorize('hr', 'admin'), [param('id').isMongoId().withMessage('Invalid application ID'), body('status').isIn(['submitted', 'under_review', 'rejected', 'interview_passed', 'offer_extended', 'offer_accepted', 'offer_declined', 'interview_scheduled']).withMessage('Invalid status'), body('notes').optional().isLength({
  max: 1000
  ...
  try {
    ...
    if (requesterRole === 'hr') {
      const hrOk = status === 'interview_scheduled' || status === INTERVIEW_PASSED && oldStatus === 'interview_confirmed' || status === 'rejected' && ['submitted', 'under_review', 'interview_scheduled', 'interview_confirmed'].includes(oldStatus);
      if (!hrOk) {
        return res.status(403).json({
          success: false,
          message: 'HR không được phép cập nhật trạng thái này cho hồ sơ hiện tại.'
        });
      }
    }
    ...
    if (status === 'interview_scheduled') {
      ...
      application.interviewInvite.hrNote = normalizedNotes;
...
router.get('/:id/resume', auth, authorize('hr', 'admin'), [param('id').isMongoId().withMessage('Invalid application ID')], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
...
router.post('/:id/ai-feedback-analysis', auth, authorize('hr', 'admin'), aiAnalysisLimiter, [param('id').isMongoId().withMessage('Invalid application ID')], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
