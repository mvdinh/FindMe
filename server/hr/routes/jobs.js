...
const ALLOWED_JOB_TYPES = ['Full-time', 'Part-time', 'Internship', 'Contract', 'Freelance', 'Temporary', 'Toàn thời gian', 'Bán thời gian', 'Thực tập', 'Hợp đồng', 'Tự do', 'Thời vụ'];
router.get('/', auth, authorize('hr', 'admin'), requireCompany, [query('page').optional().isInt({
  min: 1
  ...
  min: 1,
  max: 200
}).withMessage('Search term must be between 1 and 200 characters'), query('status').optional().isIn(['active', 'inactive', 'draft', 'closed']).withMessage('Invalid status'), query('department').optional().isLength({
  min: 1,
  max: 100
}).withMessage('Department must be between 1 and 100 characters'), query('employmentType').optional().isIn(['full-time', 'part-time', 'contract', 'temporary', 'internship']).withMessage('Invalid employment type'), query('sortBy').optional().isIn(['createdAt', 'title', 'department', 'applicants']).withMessage('Invalid sort field'), query('sortOrder').optional().isIn(['asc', 'desc']).withMessage('Sort order must be asc or desc')], getJobs);
router.post('/', auth, authorize('hr', 'admin'), requireCompany, [body('title').notEmpty().withMessage('Job title is required').isLength({
  min: 3,
  max: 200
...
router.post('/:id/status-request', auth, authorize('hr'), requireCompany, [param('id').isMongoId().withMessage('Invalid job ID'), body('requestedStatus').isIn(['active', 'closed', 'inactive', 'draft']).withMessage('Invalid requested status'), body('message').trim().isLength({
  min: 10,
  max: 2000
...
router.get('/:id', auth, authorize('hr', 'admin'), requireCompany, [param('id').isMongoId().withMessage('Invalid job ID')], getJobById);
router.put('/:id', auth, authorize('hr', 'admin'), requireCompany, [param('id').isMongoId().withMessage('Invalid job ID'), body('title').optional().isLength({
  min: 3,
  max: 200
  ...
  min: 2,
  max: 200
}).withMessage('Location must be between 2 and 200 characters'), body('experienceLevel').optional().isLength({
  min: 1,
  max: 100
}).withMessage('Invalid experience level'), body('status').optional().isIn(['active', 'inactive', 'draft', 'closed']).withMessage('Invalid status'), body('applicationDeadline').optional().isISO8601().withMessage('Application deadline must be a valid date'), body('atsEnabled').optional().isBoolean().withMessage('atsEnabled must be boolean'), body('atsResumeThreshold').optional().isFloat({
  min: 0,
  max: 100
...
router.delete('/:id', auth, authorize('hr', 'admin'), requireCompany, [param('id').isMongoId().withMessage('Invalid job ID')], deleteJob);
router.patch('/:id/status', auth, authorize('hr', 'admin'), requireCompany, [param('id').isMongoId().withMessage('Invalid job ID'), body('status').isIn(['active', 'inactive', 'closed']).withMessage('Status must be active, inactive, or closed')], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    if (status !== previousStatus) {
      job.lastStatusActorRole = String(req.user.role || '').toLowerCase() === 'admin' ? 'admin' : 'hr';
    }
...
router.get('/:id/applications', auth, authorize('hr', 'admin'), requireCompany, [param('id').isMongoId().withMessage('Invalid job ID'), query('page').optional().isInt({
  min: 1
}).withMessage('Page must be a positive integer'), query('limit').optional().isInt({
  min: 1,
  max: 50
